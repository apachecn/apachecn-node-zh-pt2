...
与其他编程技术不同，Node.js 没有附带沉重的标准库。node.js 的核心模块是最少的，其余的可以通过Node包管理器(NPM)注册中心挑选。主要的核心模块、类、方法和事件包括:
...
*   [`npmjs.org`](https://npmjs.org/) :Node包管理器注册表
...
Node程序包管理器(或称 NPM)为您管理依赖关系并安装模块。Node.js 安装自带 NPM，其网址为 [`npmjs.org`](http://npmjs.org/) 。
