...
在这一章中，我们将看看 Node.js 为创建 web 应用提供的*核心*功能。然后我们将回顾 *connect* 中间件框架，它允许您创建可重用的 web 服务器组件。最后，我们将看看用 *HTTPS* 保护你的Web 服务器。
...
一个效用函数是`listen` 。我们将在内部调用`http.createServer`，用它注册 connect dispatcher，就像我们之前展示的那样，(即`http.createServer(app))`，最后调用创建的服务器的`listen`函数。所以你可以简单地做清单 6-18 所示的事情，但是知道它仍然是Node核心`http`之上的一个调度器是很有用的。
