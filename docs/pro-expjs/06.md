...
![Image](img/sq.jpg) **提示**HTTP 方法是每个 HTTP(S)请求的特殊属性，类似于它的头或主体。在浏览器中打开 URL 是 GET 请求，提交表单是 POST 请求。其他类型的请求，如 PUT、DELETE、PATCH 和 OPTIONS，只能通过 CURL、Postman 或定制的应用程序(前端和后端)等特殊客户端获得。
