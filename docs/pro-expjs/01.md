...
Express.js 是一个Node包管理器(NPM 或`npm`)模块，它依赖于您的应用。这意味着每个用/on Express.js 构建的项目都需要在本地`node_modules`文件夹中有框架的源文件(不是全局的！).为此，您可以像安装任何其他 NPM 模块一样安装 Express.js，使用`$ npm install`，例如`$ npm install express@4.2.0`。
