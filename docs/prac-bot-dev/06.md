...
我们在构建新的 bot 实例时实例化的 ChatConnector 类会安装默认的 MemoryBotStorage 实例，这对于开发来说是一个很好的选择。SDK 允许我们提供自己的实现来替换默认功能，这是您在生产部署中最想做的事情，因为这可以确保在实例重新启动时存储状态，而不是删除状态。例如，微软提供了两个额外的接口实现，一个是 Azure Cosmos DB <sup>[1](#Fn1)</sup> 的 NoSQL 实现，另一个是 Azure Table Storage 的实现。 <sup>[2](#Fn2)</sup> 两者都是可以通过 Azure 门户获得的 Azure 服务。你可以在 botbuilder-azure Node包中找到这两个存储实现，记录在 [`https://github.com/Microsoft/BotBuilder-Azure`](https://github.com/Microsoft/BotBuilder-Azure) 。您还可以编写自己的 IBotStorage 实现，并将其注册到 SDK。编写自己的实现就是遵循简单的 IBotStorage 接口。
