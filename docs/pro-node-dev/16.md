...
![image](images/sq.jpg) **注意**正如`cluster.isMaster`标识主进程一样，`cluster.isWorker`标识子进程。
...
![image](images/sq.jpg) **注** `cluster.workers`仅在主流程中可用。然而，每个工作进程可以通过`cluster.worker`属性引用自己的`worker`对象。
