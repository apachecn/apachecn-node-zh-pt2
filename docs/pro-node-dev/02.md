...
这一章已经涵盖了大量的材料——这是必须的。开发节点应用的很大一部分是使用`npm`和第三方包。从本章开始，你应该已经很好地掌握了`npm`、`require()`函数、`package.json`文件和模块创作。虽然整个软件包系统不能在一章中全面介绍，但是你现在应该知道足够的知识来完成本书的其余部分。通过阅读在线文档来填补知识上的空白。