...
流在概念上非常相似。例如，把给草坪浇水比作呼唤`console.log()`。在这种情况下，节点应用充当水源。当调用`console.log()`时，水被打开，信息流经标准输出流。此时，节点不再关心数据会发生什么。`stdout`流将数据传送到目的地。在这种情况下，目的地(喷洒器)几乎可以是任何东西——终端窗口、文件、另一个程序。
...
在节点应用中，可以使用`pipe()`方法将流连接在一起，该方法有两个参数:一个作为数据目的地的必需的可写流和一个用于传入选项的可选对象。在清单 7-15 中的简单例子中，从`stdin`到`stdout`创建了一个管道。当这个程序运行时，它监听用户的输入。当按下`Enter`键时，用户输入的任何数据都会回显到`stdout`。
