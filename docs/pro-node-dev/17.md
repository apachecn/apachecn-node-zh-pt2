...
![image](images/sq.jpg) **注**一段 JSON 的根几乎总是一个对象。然而，这不是绝对的要求。顶层也可以是数组。
...
![image](images/sq.jpg) **注意**`JSON.parse()`和`JSON.stringify()`都是可以抛出异常的同步方法。因此，这些方法的任何使用都应该包装在一个`try...catch`语句中。
