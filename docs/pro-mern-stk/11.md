...
这两种选择都会带来问题。打破这个僵局的推荐方法是使用`react-router-bootstrap`包，它提供了一个名为`LinkContainer`的包装器，充当 React 路由器的`NavLink`，同时让它的孩子拥有自己的渲染。我们可以将一个没有`href`的`NavItem`作为子Node放在`LinkContainer`上，让父Node`LinkContainer`处理到路由的路径。
