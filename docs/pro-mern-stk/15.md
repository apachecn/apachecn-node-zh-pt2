...
事实上，许多 web 应用在 Heroku 上共享相同的资源。不仅仅是计算资源，它们还共享 IP 地址。因此，有必要为每个请求指定*该请求应该到达哪个*应用，以便它可以被路由到适当的应用。这是使用 HTTP 请求中的`Host`头来完成的。这项技术被称为基于*名字的虚拟主机*，它也受到流行的网络服务器如 Apache 和 nginx 的支持。
