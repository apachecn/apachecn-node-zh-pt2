...
现在，让我们将应用程序的主页分成两个部分:一个标题部分包含一个导航栏，其中包含指向不同视图的超链接；一个内容部分，它将根据所选的超链接在两个视图之间切换。无论显示何种视图，导航栏都将保持不变。将来，我们可能会在内容部分看到其他视图。让我们为内容创建一个组件，并把它放在目录`ui/src`下名为`Contents.jsx`的文件中。该组件将负责视图之间的切换。
