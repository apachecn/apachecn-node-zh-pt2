...
在代码编辑器中，打开根目录中的`package.json`。在依赖项列表的开始，您将添加 AWS SDK Node模块和 multer，因此您的依赖项 JSON 应该类似于清单 [5-1](#FPar3) 。
...
`aws-sdk`和`multer`Node模块及其各自的依赖项应该开始下载，这将向控制台打印诸如清单 [5-2](#FPar4) 之类的内容。
