...
在您的解决方案中，您构建了一个路由器，它可以处理从客户端提供给它的任何路径(' * ')，但是可以通过单独处理不同的协议来处理不同的路由。这意味着，如果您的应用中有一个逻辑分离，比如一个用于用户更新的 API 和一个用于产品更新的 API，您可以用一个单独的协议轻松地将它们分开。您只需在客户机上创建一个新的 WebSocket，它指向您的服务器并为每一项传递特定的协议。
